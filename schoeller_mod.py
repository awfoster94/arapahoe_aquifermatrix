...
def plot(df, 
    ...
    ax.set_xticklabels(['Ca$^{2+}$', 'Mg$^{2+}$', 'Na$^+$', 'K$^+$', 
                        'Cl$^-$', 'SO$_4^{2-}$', 'HCO$_3^-$'])
    ax.set_ylabel('$\dfrac{meq}{L}$', fontsize=8, weight='normal')
    ax.set_xlabel('         cations                                                                   anions', fontsize = 8, weight = 'normal', style='italic')
    ...
    for xtick in [1, 2, 3, 4, 5, 6, 7]:
        plt.axvline(xtick, linewidth=0.5, color='k', linestyle='--', alpha=0.25)
            
    # Create the legend
    plt.legend(bbox_to_anchor=(1.05, 1.0175), markerscale=1,
               frameon=True, edgecolor = 'black', title='Legend', fontsize=8, 
